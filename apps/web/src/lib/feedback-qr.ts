...
export async function createFeedbackQrPoster(url: string, enterpriseName: string): Promise<Blob> {
  ...
  await QRCode.toCanvas(qr, url, { errorCorrectionLevel: "H", margin: 4, width: 1120, color: { dark: "#173E30", light: "#FFFFFF" } });
  ...
  context.beginPath(); context.roundRect(48, 48, 1704, 2304, 40); context.stroke();
  ...
  nameLines.forEach((line, index) => context.fillText(line, 900, 365 + index * (size + 10)));
  context.fillStyle = "#173E30"; context.font = "96px Georgia, serif";
  context.fillText("Tell us how we did,", 900, 610);
  context.fillText("we’re listening", 900, 730);
  context.fillStyle = "#FFFFFF";
  context.beginPath(); context.roundRect(300, 840, 1200, 1200, 40); context.fill();
  context.imageSmoothingEnabled = false;
  context.drawImage(qr, 340, 880, 1120, 1120);
  // A small central logo, high error correction, and an intact quiet zone preserve scanability.
  context.fillStyle = "#FFFFFF";
  context.fillRect(777, 1370, 246, 140);
  context.imageSmoothingEnabled = true;
  drawLogo(900, 1440, 210);
  context.fillStyle = "#365444"; context.font = "46px Arial, sans-serif";
  context.fillText("Please scan the QR to", 900, 2165);
  context.fillText("submit your feedback.", 900, 2225);
