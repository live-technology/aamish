...
describe("guest feedback", () => {
  test("allows a rating without text, voice or identity", () => {
    expect(validateGuestFeedback(form({ rating: "5" }))).toEqual({ rating: 5, review: null, anonymous: false, name: null, phone: null });
  });
  ...
  test("normalizes optional contact details and bounds content", () => {
