...
export type EditableEnterprise = { id:string;name:string;slug:string;status:string;poc_name:string;poc_phone:string;poc_email:string;location_count:number;admin_count:number;locations:EnterpriseLocationRecord[] };
...
export function validateEnterpriseEdit(value: EnterpriseEditPayload) {
  ...
  if (value.locations.some((location) => !location.name.trim() || !location.code.trim() || !location.address.trim())) return "MISSING_REQUIRED_FIELDS";
  const codes = value.locations.map((location) => location.code.trim().toLowerCase());
  if (new Set(codes).size !== codes.length) return "DUPLICATE_LOCATION_CODE";
