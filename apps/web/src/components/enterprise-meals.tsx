...
import { CalendarDays, MapPin, Search, Utensils } from "lucide-react";
...
export type EnterpriseMealRow = { schedule_id: string; schedule_date: string; cutoff_time: string; status: string; location_name: string; option_label: string; menu_title: string; order_count: number };
...
function groupMeals(rows: EnterpriseMealRow[]) {
  ...
  for (const row of rows) {
    ...
    destination.options.push({ label: row.option_label, menu: row.menu_title, orders: row.order_count });
  }
