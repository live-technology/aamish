...
export function EnterpriseEditor({ enterprise, onClose, onSaved }: { enterprise: EditableEnterprise; onClose: () => void; onSaved: () => Promise<void> }) {
  const [draft, setDraft] = useState<EnterpriseEditPayload>({ logoUrl: enterprise.logo_url ?? null, id: enterprise.id, name: enterprise.name, pocName: enterprise.poc_name, pocPhone: enterprise.poc_phone, pocEmail: enterprise.poc_email, status: enterprise.status, locations: enterprise.locations.map((item) => ({ id: item.id, name: item.name, code: item.code, address: item.address, isActive: item.is_active })) });
  ...
  const dialogRef = useModalDialog<HTMLElement>(true, onClose, saving || resetting || uploadingLogo);
  ...
  async function submit(event: FormEvent) {
    ...
    if (saving || resetting || uploadingLogo) return;
  ...
  return <div className={styles.dialogBackdrop} style={{ zIndex: 80 }}><section ref={dialogRef} className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="enterprise-edit-title" tabIndex={-1}>
    <header className={styles.dialogHeader}><div><p>Edit enterprise</p><h2 id="enterprise-edit-title">{enterprise.name}</h2><span>The URL slug and administrator username remain unchanged.</span></div><IconButton aria-label="Close enterprise editor" onClick={onClose} disabled={saving || resetting || uploadingLogo}><X size={19} /></IconButton></header>
    <form className={styles.dialogForm} onSubmit={submit}><div className={styles.dialogBody}>
      ...
      <EnterpriseLogoField value={draft.logoUrl} onChange={value => update("logoUrl", value)} onBusyChange={setUploadingLogo} disabled={saving || resetting} />
    ...
    </div><footer className={styles.dialogFooter}><Button type="button" variant="secondary" onClick={onClose} disabled={saving || resetting || uploadingLogo}>Cancel</Button><Button loading={saving} loadingLabel="Saving changes…" disabled={resetting || uploadingLogo}>Save changes</Button></footer></form>
  </section></div>;
}
