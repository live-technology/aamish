...
describe("service calendar contracts", () => {
  test("validates service details and distinct menu options by step", () => {
    expect(scheduleStepError(1, { enterpriseId: "", scheduleDate: "", cutoffTime: "", menuIds: [""] })).toContain("organization");
    expect(scheduleStepError(2, { enterpriseId: "e-1", scheduleDate: "2026-09-02", cutoffTime: "10:00", menuIds: ["m-1", "m-1"] })).toContain("different package");
    expect(scheduleStepError(2, { enterpriseId: "e-1", scheduleDate: "2026-09-02", cutoffTime: "10:00", menuIds: ["m-1", "m-2"] })).toBe("");
  });
