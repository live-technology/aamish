...
import { BarChart3, CalendarDays, MessageSquareText, Search, ShieldAlert, Star, X } from "lucide-react";
...
export function QualityDashboard({ fullName, reviews, initialIssues }: { fullName: string; reviews: ReviewRow[]; initialIssues: QualityIssue[] }) {
  ...
  return <AppShell workspace="Aamish operations" fullName={fullName} roleLabel="Aamish administrator" currentPath="/admin/quality" navigation={superAdminNavigation}>
    ...
    <section className={styles.summary} aria-label="Quality totals"><Summary value={reviews.length ? average.toFixed(1) : "—"} label="45-day CSAT" /><Summary value={reviews.length} label="Meal reviews" /><Summary value={openIssues.length} label="Open food issues" /><Summary value={unclassified.length} label="Product bugs to classify" /></section>
