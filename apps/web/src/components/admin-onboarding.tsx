...
import { EnterpriseEditor } from "@/components/enterprise-editor-access";
...
export function AdminOnboarding({ fullName, initialEnterprises, startOpen = false }: { fullName: string; initialEnterprises: Enterprise[]; startOpen?: boolean }) {
  ...
  return <AppShell workspace="Aamish operations" fullName={fullName} roleLabel="Aamish administrator" currentPath="/admin/organizations" navigation={superAdminNavigation}>
    <PageHeader eyebrow="Organization setup" title="Organizations" description="Manage enterprise access, delivery locations, and the administrator for each client." actions={<Button onClick={openDialog}><Plus size={17} aria-hidden="true" />Add enterprise</Button>} />
...
function AdministratorStep({ draft, errors, update, setUsernameEdited }: { draft: EnterpriseDraft; errors: Record<string, string>; update: <K extends keyof EnterpriseDraft>(field: K, value: EnterpriseDraft[K]) => void; setUsernameEdited: (value: boolean) => void }) {
  return <section className={styles.stepContent}><div className={styles.stepHeading}><span>3 of 4</span><h3>Administrator</h3><p>Create the person who will manage employees and view meal activity for this enterprise.</p></div><div className={styles.fieldGrid}><TextField autoFocus label="Administrator name" name="adminFullName" value={draft.adminFullName} onChange={(event) => update("adminFullName", event.target.value)} placeholder="Full name" error={errors.adminFullName} required /><TextField label="Username" name="adminUsername" value={draft.adminUsername} onChange={(event) => { setUsernameEdited(true); update("adminUsername", event.target.value); }} description="Suggested from the enterprise name; edit if needed." error={errors.adminUsername} required /><TextField className={styles.wideField} label="Temporary password" name="adminPassword" type="password" minLength={8} autoComplete="new-password" value={draft.adminPassword} onChange={(event) => update("adminPassword", event.target.value)} description="At least 8 characters. The administrator must replace it at first sign-in." error={errors.adminPassword} required /></div></section>;
}

function ReviewStep({ draft }: { draft: EnterpriseDraft }) {
  return <section className={styles.stepContent}><div className={styles.stepHeading}><span>4 of 4</span><h3>Review and create</h3><p>Confirm the enterprise, delivery locations, and administrator.</p></div><div className={styles.reviewGrid}><ReviewCard title="Company"><strong>{draft.name}</strong><span>{draft.pocName}</span><span>{draft.pocPhone} · {draft.pocEmail}</span></ReviewCard><ReviewCard title={`Delivery locations · ${draft.locations.length}`}>{draft.locations.map((location, index) => <div className={styles.reviewLocation} key={`${location.name}-${index}`}><strong>{location.name}</strong><span>Code assigned automatically · {location.address}</span></div>)}</ReviewCard><ReviewCard title="Administrator"><strong>{draft.adminFullName}</strong><span>@{draft.adminUsername}</span><span>Temporary password must be changed at first sign-in</span></ReviewCard></div><Alert tone="info" title="Creation is atomic">The enterprise, locations, and administrator are saved together. If one part fails, none of them are created.</Alert></section>;
}
...
export function SuccessHandoff({ created, copyUsername, close }: { created: CreatedEnterprise; copyUsername: () => void; close: () => void }) { return <div className={styles.successState}><span><Check size={27} /></span><h3>Enterprise created</h3><p>The organization, its delivery locations, and administrator are ready.</p><div className={styles.credentialHandoff}><small>Administrator username</small><strong>{created.enterpriseAdminUsername}</strong><Button type="button" variant="secondary" onClick={copyUsername}><Copy size={16} />Copy username</Button></div><Alert tone="warning" title="Share the password securely">For safety, the temporary password is not displayed again. It must be replaced at first sign-in.</Alert><Button type="button" onClick={close}>Done</Button></div>; }
