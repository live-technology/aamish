...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (session?.role !== "SUPER_ADMIN") return Response.json({ error: "Access denied." }, { status: 403, headers });
    const { id } = await context.params;
    if (!isRequestId(id)) return new Response(null, { status: 404, headers });
    const rows = await db()`SELECT audio_public_id, audio_format FROM meal_requests WHERE id = ${id}`;
    ...
    if (!response.ok) {
      log("meal_request.audio_unavailable", { requestId, mealRequestId: id, upstreamStatus: response.status });
    ...
    log("meal_request.audio_failed", { requestId });
