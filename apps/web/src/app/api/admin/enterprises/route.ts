...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const result = await db().begin(async (transaction) => {
      ...
      const current = await transaction<{id:string;code:string}[]>`SELECT id,code FROM delivery_locations WHERE enterprise_id=${payload.id} FOR UPDATE`;
      ...
      const newLocations = payload.locations.filter((location) => !location.id);
      const generatedCodes = assignLocationCodes(newLocations.map((location) => location.name), current.map((location) => location.code));
  ...
  try {
    ...
    const result = await db().begin(async (transaction) => {
      ...
      for (const [index, location] of draft.locations.entries()) {
        await transaction`INSERT INTO delivery_locations (enterprise_id, name, code, address) VALUES (${enterpriseId}, ${location.name.trim()}, ${locationCodes[index]}, ${location.address.trim()})`;
      }
      const users = await transaction<{ id: string }[]>`
        INSERT INTO app_users (username, password_hash, full_name, role, enterprise_id, must_change_password)
        VALUES (${draft.adminUsername.trim()}, crypt(${draft.adminPassword}, gen_salt('bf')), ${draft.adminFullName.trim()}, 'ENTERPRISE_ADMIN', ${enterpriseId}, TRUE) RETURNING id
      `;
...
function enterpriseDraft(body: unknown): EnterpriseDraft | null {
  ...
  return {
    ...
    locations: locations.map((location) => ({ name: location.name as string, code: typeof location.code === "string" ? location.code : "", address: location.address as string })),
